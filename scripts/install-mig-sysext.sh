...
#
# The one-liner is get.sh on main, which runs this script from the newest
# release a hardware test approved for the box's TrueNAS train:
...
#                         With no --release the newest release a hardware test
#                         approved for this box's TrueNAS train is used, and the
#                         install stops if there is none (get.sh, the one-liner,
#                         picks it the same way).
...
# Resolve the release to install into RELEASE_TAG (no-op when --release
# pinned one): the newest release a hardware test approved for this box's
# TrueNAS train, never an unapproved one. Callers choose their own severity on
# failure; the selection has already explained why on stderr.
resolve_release_tag() {
    [ -n "$RELEASE_TAG" ] && return 0
    local tag
    tag=$(approved_release_tag) || return 1
    RELEASE_TAG="$tag"
    echo "Resolved release: ${RELEASE_TAG} (newest approved for this TrueNAS train)"
}
...
if [ -z "$MIG_SRC" ]; then
    resolve_release_tag || {
        echo "ERROR: cannot resolve a release approved for this TrueNAS train; pass --release=TAG to pin one explicitly" >&2
        exit 1
    }
...
# network fetch remains for raws that predate the bundling, pinned to the
# release (--release, or the newest one approved for this TrueNAS train). No
# fallback to main: main's copy is not the one a hardware test approved.
# Honors --dry-run. Returns non-zero if it can't obtain the file.
stage_mig_preinit() {
    local dest="$1" dir url
    ...
    if $DRY_RUN; then
        echo "[dry-run] would: fetch nvidia-mig-preinit.sh from release ${RELEASE_TAG} to ${dest}" >&2
        return 0
    fi
    url="https://raw.githubusercontent.com/${REPO}/${RELEASE_TAG}/scripts/nvidia-mig-preinit.sh"
...
if ! stage_mig_preinit "$PREINIT_DEST"; then
    ...
    echo "  (not found alongside this script or in the raw, and could not be fetched from release ${RELEASE_TAG:-<none>})" >&2
    exit 1
fi
