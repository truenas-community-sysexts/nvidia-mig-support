...
- **Order `nvidia-mig-setup.service` `Before=docker.service`.** dockerd now waits for MIG instance creation, so the `unless-stopped` restart lands on devices that already exist. This matches the convention the sibling sysexts already use (`hailo-load.service` / `coral-load.service` both declare `Before=docker.service`), so all four order docker the same way. Delivered via the sysext, so it travels with the unit and survives TrueNAS updates; loaded before docker every boot because `systemd-sysext` finishes before `basic.target` and `EXTENSION_RELOAD_MANAGER=1` reloads PID 1 after the merge, while `docker.service` is a regular `DefaultDependencies=yes` service ordered after `basic.target`.
- **`install … --check`** gains a pass/fail line verifying `docker.service` is actually ordered after `nvidia-mig-setup.service` (queries `systemctl show docker.service -p After`, so it reflects the live loaded config — the `Before=` edge surfaces symmetrically there).
- **Deadlock-safe by construction.** Instance creation runs before `nvidia-mig-setup`'s bounded (≤25 s) middleware-ready poll, and the service never blocks boot, so it always exits and releases docker even if middleware readiness transitively depended on docker.
- **Known tradeoff:** docker now waits for the whole MIG service (incl. the middleware poll), delaying *all* app startup by tens of seconds on boot. Correctness over speed for now; a follow-up can split instance creation from app remap and gate docker on only the former. Tracked in [docs/mig-persistence.md](docs/mig-persistence.md).
- **Docs:** new troubleshooting section (crash-on-boot fingerprint + fix), boot-timing rationale in `mig-persistence.md`, and the `Before=docker.service` ordering noted in the `architecture.md` file tree and boot diagram.
