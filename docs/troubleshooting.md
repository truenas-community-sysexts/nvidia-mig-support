...
**Cause:** You just ran the full-driver install (`install-nvidia-sysext.sh`) or `recover-stock-nvidia.sh --install`. The new userspace libraries are live (sysext is merged), but the previous driver's kernel modules are still loaded in kernel memory.
...
Hot-unload (no reboot) is technically possible — `rmmod nvidia_uvm nvidia_drm nvidia_modeset nvidia` after stopping Docker — but the reboot path is more reliable. The full-driver install script prints an explicit reboot-required warning for this reason.
...
for s in json.load(sys.stdin):
    ...
    if 'nvidia-mig-setup' in cmd or 'nvidia-preinit-full' in cmd:
        print(s)
"
```
...
**Cause:** You never had a stock-driver backup, or an earlier install/restore cycle wiped it. The lightweight path doesn't need one (it never touches stock `nvidia.raw`), but `install-nvidia-sysext.sh` and `uninstall-nvidia-sysext.sh` both depend on having one.
...
## Lightweight install fails pre-flight with `stock driver X.Y.Z is below the minimum-validated 570.x`
