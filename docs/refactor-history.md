...
>
> **Driver path removed (later).** The whole driver side — the `--with-driver` mode, on-host `nvidia.raw` build (`build-nvidia-sysext.sh`/`build-on-host.sh`), stock recovery, driver PREINIT, and the upstream-version poller — was subsequently **extracted into a separate project, [nvidia-driver-support](https://github.com/truenas-community-sysexts/nvidia-driver-support)**. This repo is now MIG-only: it builds and installs just `nvidia-mig.raw` on top of whatever driver is present. Everything below describing a second full-driver sysext is therefore history, not current behavior.
